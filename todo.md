...
- 2026-06-15: Reduced noisy Gallop metadata influence in `artifacts/api-server/src/lib/forecasting.ts`, tightened blank/unknown favourite parsing, shifted learning feedback toward known top-four coverage instead of winner-only factor correction, and batched historical learning rebuild queries. Verified with `corepack pnpm --filter @workspace/api-server run typecheck` (exit 0), `corepack pnpm --filter @workspace/api-server run build` (exit 0), and a live `rebuildLearningFeedbackFromHistory()` run against the configured second `DATABASE_URL`, which applied 233 historical results and skipped 9.
